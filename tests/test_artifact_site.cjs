...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 768, 390, 320]) {
      ...
      assert.equal(await page.locator('#install-tag').inputValue(), 'macos-v0.1.0-rc.5');
      assert.match(await page.locator('#install-command').textContent(), /--source oss --tag macos-v0.1.0-rc.5/);
      assert.ok(!/--install-system-deps/.test(await page.locator('#install-command').textContent()));
      await page.locator('[data-language="en"]').click();
      assert.match(await page.locator('#install-command-en').textContent(), /install-en\.sh.*--source github --tag macos-v0.1.0-rc.5/);
      ...
      assert.equal(await page.locator('.musl-release a').getAttribute('href'), 'https://github.com/insightos-community/quick-start/releases/tag/musl-v0.1.0-3');
