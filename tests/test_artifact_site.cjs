...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 768, 390, 320]) {
      ...
      assert.match(await page.locator('#musl-command').textContent(), /install\.sh.*[\s\S]*--tag musl-v/);
      assert.match(await page.locator('#musl-install').textContent(), /626 MiB/);
      ...
      assert.equal(await page.locator('[data-platform="macos"]').getAttribute('aria-pressed'), 'true');
      assert.match(await page.locator('#uninstall-local-command').textContent(), /semantic-macos\/bin\/semanticctl/);
      assert.equal(await page.locator('#install-tag').inputValue(), 'macos-v0.1.0-rc.4');
      assert.match(await page.locator('#install-command').textContent(), /--source oss --tag macos-v0.1.0-rc.4/);
      assert.ok(!/--install-system-deps/.test(await page.locator('#install-command').textContent()));
      await page.locator('[data-language="en"]').click();
      assert.match(await page.locator('#install-command-en').textContent(), /install-en\.sh.*--source github --tag macos-v0.1.0-rc.4/);
      assert.equal(await page.locator('#install-platform').inputValue(), 'macos');
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), `macOS controls overflow at ${width}px`);
      await page.locator('#install-tag').fill('macos-v0.1.0-rc.1');
      assert.match(await page.locator('#install-command-en').textContent(), /--tag macos-v0.1.0-rc.1/);
      ...
      assert.match(await page.locator('#install-command-en').textContent(), /--tag musl-v0.1.0-2/);
      await page.locator('[data-platform="glibc"]').click();
      await page.locator('[data-language="zh"]').click();
      assert.match(await page.locator('#install-command').textContent(), /--source oss --version stable/);
      ...
      assert.match(await page.locator('.support-note').textContent(), /Linux x86_64.*macOS 15.5/);
      ...
      assert.match(await page.locator('#musl-command').textContent(), /install-en\.sh.*[\s\S]*--tag musl-v/);
      assert.match(await page.locator('#musl-install').textContent(), /musl runtime/);
      assert.equal(await page.locator('.musl-release a').getAttribute('href'), 'https://github.com/insightos-community/quick-start/releases/tag/musl-v0.1.0-2');
