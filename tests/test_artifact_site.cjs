...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 768, 390, 320]) {
      ...
      assert.equal(await page.locator('#install-tag').inputValue(), 'macos-v0.1.0-rc.3');
      assert.match(await page.locator('#install-command').textContent(), /--source github --tag macos-v0.1.0-rc.3/);
      assert.ok(!/--install-system-deps/.test(await page.locator('#install-command').textContent()));
      await page.locator('[data-language="en"]').click();
      assert.match(await page.locator('#install-command-en').textContent(), /install-en\.sh.*--tag macos-v0.1.0-rc.3/);
