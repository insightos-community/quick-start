...
class PlatformBootstrapTests(unittest.TestCase):
    ...
    def test_tags_select_the_platform_and_release_source_in_both_languages(self):
        for script in ['install.sh', 'install-en.sh']:
            with self.subTest(script=script):
                ...
                self.assertEqual(bool(ns['use_github']), script == 'install-en.sh')
                ...
                with self.assertRaises(SystemExit):
                    self.selection(script, ['--tag', 'macos-v0.1.0-rc.4'])
                with self.assertRaises(SystemExit):
    ...
    def test_macos_system_bash_handles_empty_optional_arguments(self):
        ...
        self.assertIn('macos-v0.1.0-rc.4', result.stdout)
    ...
    def test_macos_offline_route_needs_no_host_python_and_checks_archive_integrity(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for script in ['install.sh', 'install-en.sh']:
                command = ['bash', str(ROOT / script), '--tag', 'macos-v0.1.0-rc.4', '--source', 'github', '--package', str(archive), '--sha256', digest, '--dir', str(root / 'instance with spaces'), '--no-start', '--yes']
                ...
                mirrored = command.copy(); mirrored[mirrored.index('--source')+1] = 'oss'
                result = subprocess.run(mirrored, env=env, capture_output=True, text=True)
                self.assertEqual(result.returncode, 0, result.stderr)
