...
| macOS preview | `macos-v0.1.0-rc.4` | Native Apple Silicon, macOS 15.5+; Go 1.25.8, uv 0.12.12, xmake 3.1.1 and Xcode Command Line Tools | Bundled Python 3.13.15/NumPy 2.3.5, locked macOS wheels and native applications; system CGL |
...
git checkout --detach macos-v0.1.0-rc.4
test "$(uname -s)" = Darwin
test "$(uname -m)" = arm64
...
.build-venv/bin/python artifacts/macos/build.py --version 0.1.0-rc.4 \
...
.build-venv/bin/python artifacts/macos/smoke_installer.py \
  --package dist-macos/semantic-0.1.0-rc.4-macos-arm64.tar.gz \
...
gh auth setup-git
git push origin 'macos-v0.1.0-rc.4^{commit}:refs/heads/reproduce/macos-installer'
