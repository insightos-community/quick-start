...
git checkout --detach macos-v0.1.0-rc.1
test "$(uname -s)" = Darwin
test "$(uname -m)" = arm64
...
.build-venv/bin/python artifacts/macos/build.py --version 0.1.0-rc.1 \
...
.build-venv/bin/python artifacts/macos/smoke_installer.py \
  --package dist-macos/semantic-0.1.0-rc.1-macos-arm64.tar.gz \
...
gh auth setup-git
git push origin 'macos-v0.1.0-rc.1^{commit}:refs/heads/reproduce/macos-installer'
