...
The Ability range can be reconfigured before Robot configurations are created. Afterwards, use a new installation directory to change that range; changes that invalidate existing allocations are rejected. A new Mac can export the default template without system Python; existing Macs use bundled Python for export and reconfiguration.
